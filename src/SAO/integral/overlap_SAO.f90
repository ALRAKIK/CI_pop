subroutine overlap_matrix_SAO(n_atoms,nbas,atoms,AO,S_SAO_diag)
      ...
      double precision  ,intent(out) :: S_SAO_diag(nbas,nbas)
      ...
      double precision    :: S(nbas,nbas) , SAO(nbas,nbas) , S_temp(nbas,nbas)
          ...
          if (AO1%orbital =="s" .and. AO2%orbital == "s") then
            ...
            do k = 1 , size  (AO1%exponent)
              do l = 1 , size  (AO2%exponent)
                call overlap_integral_ss_SAO(r1,r2,AO1,AO2,S(i,j))
              end do 
            end do 
...
end subroutine overlap_matrix_SAO