subroutine overlap_matrix_torus(number_of_atoms,number_of_functions,atoms,AO)
      ...
      do i = 1 , index_atom1
        do j = 1 , number_of_functions
          ...
          if (AO1%orbital =="s" .and. AO2%orbital == "s") then
            ...
            do k = 1 , size  (AO1%exponent)
              do l = 1 , size  (AO2%exponent)
                call overlap_integral_ss_torus(r1,r2,AO1,AO2,overlap(i,j))
              end do 
            end do 
          ...
          if (AO1%orbital =="s" .and. AO2%orbital(:1) == "p") then
            ...
            do k = 1 , size  (AO1%exponent)
              do l = 1 , size  (AO2%exponent)
                call overlap_integral_sp_torus(r1,r2,AO1,AO2,overlap(i,j))
              end do 
            end do
          ...
          if (AO1%orbital(:1) == "p" .and. AO2%orbital == "s") then
            ...
            do k = 1, size(AO1%exponent)
              do l = 1, size(AO2%exponent)
                call overlap_integral_sp_torus(r2, r1, AO2, AO1, overlap(i,j))
              end do 
            end do
          ...
          if (AO1%orbital(:1) == "p" .and. AO2%orbital(:1) == "p") then
            ...
            do k = 1, size(AO1%exponent)
              do l = 1, size(AO2%exponent)
                call overlap_integral_pp_torus(r1, r2, AO1, AO2, overlap(i,j))
              end do 
            end do
      ...
      do i = index_atom1 + 1   , number_of_functions
        do j = index_atom1 + 1 , number_of_functions
          overlap(i,j) = overlap(i-index_atom1,j-index_atom1)
        end do 
      end do 
...
end subroutine overlap_matrix_torus
