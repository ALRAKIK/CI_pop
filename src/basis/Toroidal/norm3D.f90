subroutine normalize_basis_tor_3D()
      ...
      do
        ...
        if (lines == "$ P-TYPE FUNCTIONS") then 
          ...
          call norm_orb_tor_px_3D(n_gaussian,n_contraction,exponent,contraction,n_type,contractionN,Lx,Ly,Lz)
          write(3,'(4I4)') n_gaussian ,  n_contraction
          do i = 1 , n_gaussian
            write(3,*) exponent(i) , (contractionN(i,j),j=1,n_contraction)
          end do
          call norm_orb_tor_py_3D(n_gaussian,n_contraction,exponent,contraction,n_type,contractionN,Lx,Ly,Lz)
          write(4,'(4I4)') n_gaussian ,  n_contraction
          do i = 1 , n_gaussian
            write(4,*) exponent(i) , (contractionN(i,j),j=1,n_contraction)
          end do
          call norm_orb_tor_pz_3D(n_gaussian,n_contraction,exponent,contraction,n_type,contractionN,Lx,Ly,Lz)
          write(5,'(4I4)') n_gaussian ,  n_contraction
          do i = 1 , n_gaussian
            write(5,*) exponent(i) , (contractionN(i,j),j=1,n_contraction)
          end do
        ...
        sum = 0.d0
          do i = 1 , n_gaussian
            do j = 1 , n_gaussian
              ...
              const = c1*c2*Lx*Ly*Lz*(1.d0/(2.d0*gamma))
              sum  = sum + const*I_1_gamma_x*I_1_gamma_y*I_1_gamma_z
...
end subroutine  norm_orb_tor_px_3D


subroutine norm_orb_tor_py_3D(n_gaussian , n_contraction, exponent, contraction  , n_type , contractionN, Lx , Ly , Lz )
...
20    continue 
      ...
      do n = 1 , n_contraction
        do i = 1 , n_gaussian
          ...
          Norm(i) = 1.d0 / dsqrt( Lx * bessi_scaled(0,4.d0*exponent(i)/(ax2))                               &
                                * Ly * bessi_scaled(1,4.d0*exponent(i)/(ay2)) * (1.d0/(4.d0*exponent(i)))   &
                                * Lz * bessi_scaled(0,4.d0*exponent(i)/(az2))                               ) 
        ...
        do n = 1 , n_contraction
          ...
          do i = 1 , n_gaussian
            do j = 1 , n_gaussian
              ...
              I_1_gamma_x = bessi_scaled(0,2.d0*gamma/ax2)
              I_1_gamma_y = bessi_scaled(1,2.d0*gamma/ay2)
              I_1_gamma_z = bessi_scaled(0,2.d0*gamma/az2)
...
end subroutine  norm_orb_tor_py_3D


subroutine norm_orb_tor_pz_3D(n_gaussian , n_contraction, exponent, contraction  , n_type , contractionN, Lx , Ly , Lz )
...
20    continue 
      ...
      do n = 1 , n_contraction
        do i = 1 , n_gaussian
          ...
          Norm(i) = 1.d0 / dsqrt( Lx * bessi_scaled(0,4.d0*exponent(i)/(ax2))                               &
                                * Ly * bessi_scaled(0,4.d0*exponent(i)/(ay2))                               &
                                * Lz * bessi_scaled(1,4.d0*exponent(i)/(az2)) * (1.d0/(4.d0*exponent(i)))     ) 
        ...
        do n = 1 , n_contraction
          ...
          do i = 1 , n_gaussian
            do j = 1 , n_gaussian
              ...
              I_1_gamma_x = bessi_scaled(0,2.d0*gamma/ax2)
              I_1_gamma_y = bessi_scaled(0,2.d0*gamma/ay2)
              I_1_gamma_z = bessi_scaled(1,2.d0*gamma/az2)
...
end subroutine  norm_orb_tor_pz_3D