subroutine normalize_basis_tor_1D()
      ...
      do
        ...
        if (lines == "$ P-TYPE FUNCTIONS") then 
          ...
          call norm_orb_tor_p_1D(n_gaussian,n_contraction,exponent,contraction,n_type,contractionN,Lx,Ly,Lz)
          write(3,'(4I4)') n_gaussian ,  n_contraction
          do i = 1 , n_gaussian
            write(3,*) exponent(i) , (contractionN(i,j),j=1,n_contraction)
          end do
...
subroutine norm_orb_tor_p_1D(n_gaussian , n_contraction, exponent, contraction  , n_type , contractionN, Lx , Ly , Lz )
...
end subroutine  norm_orb_tor_p_1D