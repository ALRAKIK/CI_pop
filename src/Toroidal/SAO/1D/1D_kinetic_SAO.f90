subroutine kinetic_matrix_toroidal_SAO(nBas,AO,kinetic)
      ...
      double precision,intent(out) :: kinetic(nBas,nBas)
      ...
      do i = 1 , nfuc
        do j = 1 , nBas
          if (abs(kinetic(i,j)) < 1e-14) kinetic(i,j) = 0.d0 
        end do 
      end do 

      do i = nfuc + 1   , nBas
        do j = nfuc + 1 , nBas
          kinetic(i,j) = kinetic(i-nfuc,j-nfuc)
        end do 
      end do 
...
end subroutine kinetic_matrix_toroidal_SAO