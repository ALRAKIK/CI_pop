subroutine ERI_integral_4_function_toroidal_3D(one,two,three,four,value,N_max)
          ...
          do k = 1 , size(three%exponent)
            ...
            do l = 1 , size (four%exponent)
              ...
              call integrate_ERI_3D(pattern_id,mu,nu,mu_x,nu_x,phix,xpA,xpB,xqC,xqD,xa,xb,xc,xd,xp,xq,&
                                                     ...
                                                     mu_z,nu_z,phiz,zpA,zpB,zqC,zqD,za,zb,zc,zd,zp,zq,value_s,N_max)
...
subroutine integrate_ERI_3D(pattern_id,p,q,p_x,q_x,phix,xpA,xpB,xqC,xqD,xa,xb,xc,xd,xp,xq,&
      ...
      double precision                   :: Ax_cache(-2:N_max) , Bx_cache(-2:N_max)
      double precision                   :: Ay_cache(-2:N_max) , By_cache(-2:N_max)
      double precision                   :: Az_cache(-2:N_max) , Bz_cache(-2:N_max)
      ...
      ! call cache_bessel(AAx,BBx,AAy,BBy,AAz,BBz,Ax_cache,Bx_cache,Ay_cache,By_cache,Az_cache,Bz_cache,N_max)
      ...
      function f_decay_transformed(u) result(fu)

        double precision, intent(in) :: u
        double precision             :: fu
        double precision             :: t, jacobian
    
        ! Transform u -> t
        t = u / (1.0d0 - u)

        ! Jacobian factor
        jacobian = 1.0d0 / (1.0d0 - u)**2

        ! Compute C(u) for each direction
      
        CCx = 2.0d0 * u**2 / (ax * ax * (1.0d0 - u)**2)
        CCy = 2.0d0 * u**2 / (ay * ay * (1.0d0 - u)**2)
        CCz = 2.0d0 * u**2 / (az * az * (1.0d0 - u)**2)
    
        ! Evaluate your sums with these C values
        fu = S(t) * jacobian
      ...
      !sum1      = Ax_cache(0) * Bx_cache(0) * bessi_scaled(n, CCx)  
      ...
      !sum2      = Ay_cache(0) * By_cache(0) * bessi_scaled(n, CCY)
      ...
      !sum3      = Az_cache(0) * Bz_cache(0) * bessi_scaled(n, CCZ) 
...
subroutine cache_bessel(Ax,Bx,Ay,By,Az,Bz,Ax_cache,Bx_cache,Ay_cache,By_cache,Az_cache,Bz_cache,N_max)
      ...
      double precision ,intent(in)  :: Ax,Bx,Ay,By,Az,Bz
      integer          ,intent(in)  :: N_max

      integer                       :: ierr , i , n 


      double precision ,intent(out) :: Ax_cache(-2:N_max),Bx_cache(-2:N_max)
      double precision ,intent(out) :: Ay_cache(-2:N_max),By_cache(-2:N_max)
      double precision ,intent(out) :: Az_cache(-2:N_max),Bz_cache(-2:N_max)

      double precision, parameter :: UNDERFLOW_THRESHOLD = 1.0d-300
...
!      ierr = bessi_scaled_array(1, N_max, Ax, Ax_cache(1:N_max))
!      if (ierr /= 0) print *, "Warning: GSL error in Ax cache, code:", ierr
!      ierr = bessi_scaled_array(1, N_max, Bx, Bx_cache(1:N_max))
!      if (ierr /= 0) print *, "Warning: GSL error in Bx cache, code:", ierr
!      ierr = bessi_scaled_array(1, N_max, Ay, Ay_cache(1:N_max))
!      if (ierr /= 0) print *, "Warning: GSL error in Ay cache, code:", ierr
!      ierr = bessi_scaled_array(1, N_max, By, By_cache(1:N_max))
!      if (ierr /= 0) print *, "Warning: GSL error in By cache, code:", ierr
!      ierr = bessi_scaled_array(1, N_max, Az, Az_cache(1:N_max))
!      if (ierr /= 0) print *, "Warning: GSL error in Az cache, code:", ierr
!      ierr = bessi_scaled_array(1, N_max, Bz, Bz_cache(1:N_max))
!      if (ierr /= 0) print *, "Warning: GSL error in Bz cache, code:", ierr

      ! call clean_cache_after_underflow(Ax_cache, N_max)
      ! call clean_cache_after_underflow(Bx_cache, N_max)

      ! call clean_cache_after_underflow(Ay_cache, N_max)
      ! call clean_cache_after_underflow(By_cache, N_max)

      ! call clean_cache_after_underflow(Az_cache, N_max)
      ! call clean_cache_after_underflow(Bz_cache, N_max)


      !do i = -2 , N_max , 1
      !  Ax_cache(i) = bessi_scaled(i,Ax)
      !  Bx_cache(i) = bessi_scaled(i,Bx)
      !  Ay_cache(i) = bessi_scaled(i,Ay)
      !  By_cache(i) = bessi_scaled(i,By)
      !  Az_cache(i) = bessi_scaled(i,Az)
      !  Bz_cache(i) = bessi_scaled(i,Bz)
      !end do 


      ! ===== Cache Ax using recurrence =====
      Ax_cache(0) = bessi_scaled(0, Ax)  ! I_0
      Ax_cache(1) = bessi_scaled(1, Ax)  ! I_1
      
      do n = 1, N_max - 1
          ! I_{n+1} = I_{n-1} - (2n/x) * I_n
          Ax_cache(n+1) = Ax_cache(n-1) - (2.0d0*n/Ax) * Ax_cache(n)
          
          ! Check for underflow
          if (abs(Ax_cache(n+1)) < UNDERFLOW_THRESHOLD .or. Ax_cache(n+1) /= Ax_cache(n+1)) then
              Ax_cache(n+1:N_max) = 0.0d0
              exit
          endif
      enddo
      
      ! ===== Cache Bx using recurrence =====
      Bx_cache(0) = bessi_scaled(0, Bx)
      Bx_cache(1) = bessi_scaled(1, Bx)
      
      do n = 1, N_max - 1
          Bx_cache(n+1) = Bx_cache(n-1) - (2.0d0*n/Bx) * Bx_cache(n)
          if (abs(Bx_cache(n+1)) < UNDERFLOW_THRESHOLD .or. Bx_cache(n+1) /= Bx_cache(n+1)) then
              Bx_cache(n+1:N_max) = 0.0d0
              exit
          endif
      enddo
      
      ! ===== Cache Ay using recurrence =====
      Ay_cache(0) = bessi_scaled(0, Ay)
      Ay_cache(1) = bessi_scaled(1, Ay)
      
      do n = 1, N_max - 1
          Ay_cache(n+1) = Ay_cache(n-1) - (2.0d0*n/Ay) * Ay_cache(n)
          if (abs(Ay_cache(n+1)) < UNDERFLOW_THRESHOLD .or. Ay_cache(n+1) /= Ay_cache(n+1)) then
              Ay_cache(n+1:N_max) = 0.0d0
              exit
          endif
      enddo
      
      ! ===== Cache By using recurrence =====
      By_cache(0) = bessi_scaled(0, By)
      By_cache(1) = bessi_scaled(1, By)
      
      do n = 1, N_max - 1
          By_cache(n+1) = By_cache(n-1) - (2.0d0*n/By) * By_cache(n)
          if (abs(By_cache(n+1)) < UNDERFLOW_THRESHOLD .or. By_cache(n+1) /= By_cache(n+1)) then
              By_cache(n+1:N_max) = 0.0d0
              exit
          endif
      enddo
      
      ! ===== Cache Az using recurrence =====
      Az_cache(0) = bessi_scaled(0, Az)
      Az_cache(1) = bessi_scaled(1, Az)
      
      do n = 1, N_max - 1
          Az_cache(n+1) = Az_cache(n-1) - (2.0d0*n/Az) * Az_cache(n)
          if (abs(Az_cache(n+1)) < UNDERFLOW_THRESHOLD .or. Az_cache(n+1) /= Az_cache(n+1)) then
              Az_cache(n+1:N_max) = 0.0d0
              exit
          endif
      enddo
      
      ! ===== Cache Bz using recurrence =====
      Bz_cache(0) = bessi_scaled(0, Bz)
      Bz_cache(1) = bessi_scaled(1, Bz)
      
      do n = 1, N_max - 1
          Bz_cache(n+1) = Bz_cache(n-1) - (2.0d0*n/Bz) * Bz_cache(n)
          if (abs(Bz_cache(n+1)) < UNDERFLOW_THRESHOLD .or. Bz_cache(n+1) /= Bz_cache(n+1)) then
              Bz_cache(n+1:N_max) = 0.0d0
              exit
          endif
      enddo
      
      ! Fill negative indices using I_{-n} = I_n
      do n = 1, 2
          Ax_cache(-n) = Ax_cache(n)
          Bx_cache(-n) = Bx_cache(n)
          Ay_cache(-n) = Ay_cache(n)
          By_cache(-n) = By_cache(n)
          Az_cache(-n) = Az_cache(n)
          Bz_cache(-n) = Bz_cache(n)
      enddo
