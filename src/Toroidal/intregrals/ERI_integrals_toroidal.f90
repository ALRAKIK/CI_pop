subroutine ERI_integral_4_function_toroidal(one,two,three,four,value)
      ...
      double precision                :: xpA , xpB , xqC , xqD 
      ...
      do i = 1 , size  (one%exponent)
        ...
        do j = 1 , size  (two%exponent)
          ...
          !xp    = datan((alpha*dsin(ax*xa)+beta*dsin(ax*xb))/(alpha*dcos(ax*xa)+beta*dcos(ax*xb)))/ax + 0.5d0 * Lx * Heaviside(-alpha*dcos(ax*xA)-beta*dcos(ax*xB))
          ...
          do k = 1 , size(three%exponent)
            ...
            do l = 1 , size (four%exponent)
              ...
              !xq    = datan((gamma*dsin(ax*xC)+delta*dsin(ax*xD))/(gamma*dcos(ax*xC)+delta*dcos(ax*xD)))/ax + 0.5d0 * Lx * Heaviside(-gamma*dcos(ax*xC)-delta*dcos(ax*xD)) 
              ...
              call integrate_ERI_mod_mod(pattern_id,mu,nu,mu_x,nu_x,xp-xq,xpA,xpB,xqC,xqD,xa,xb,xc,xd,xp,xq,value_s)
...
subroutine integrate_ERI_mod_mod(pattern_id,p,q,p_x,q_x,xpq,xpA,xpB,xqC,xqD,xa,xb,xc,xd,xp,xq,result)
      ...
      COMPLEX(KIND=KIND(1.0D0))            :: termAm ,termBm 
      ...
      case (0000) ! | s   s   s   s    ( 1 ) 
        ...
        do n      = 1 , Nmax 
          ...
          termAn  = bessi_scaled(n, A)
          termBn  = bessi_scaled(n, B)
          termAm  = bessi_scaled(n, A)
          termBm  = bessi_scaled(n, B)
          termc   = bessi_scaled(n, C) 
          term1   = exp(I_dp*dble(n)*ax*xpq) * termC * termAn * termBn
          term2   = exp(I_dp*dble(m)*ax*xpq) * termC * termAm * termBm
         ...
         do n      = 1 , Nmax
           ...
           termBn  = (-n*I_dp/B*cos(xqD)*bessi_scaled(n,B)+(0.5d0*(bessi_scaled(n-1,B)+bessi_scaled(n+1,B)))*sin(xqD))/ax
           termAm  = bessi_scaled(n, A)
           termBm  = (-m*I_dp/B*cos(xqD)*bessi_scaled(n,B)+(0.5d0*(bessi_scaled(n-1,B)+bessi_scaled(n+1,B)))*sin(xqD))/ax
           termc   = bessi_scaled(n, C) 
           term1   = exp(I_dp*dble(n)*ax*xpq) * termC * termAn * termBn
           term2   = exp(I_dp*dble(m)*ax*xpq) * termC * termAm * termBm
           if (abs(term1) < tol)  exit
         ...
         do n      = 1 , Nmax
           ...
           termBn  = (-n*I_dp/B*cos(xqC)*bessi_scaled(n,B)+(0.5d0*(bessi_scaled(n-1,B)+bessi_scaled(n+1,B)))*sin(xqC))/ax
           termAm  = bessi_scaled(n, A)
           termBm  = (-m*I_dp/B*cos(xqC)*bessi_scaled(n,B)+(0.5d0*(bessi_scaled(n-1,B)+bessi_scaled(n+1,B)))*sin(xqC))/ax
           termc   = bessi_scaled(n, C) 
           term1   = exp(I_dp*dble(n)*ax*xpq) * termC * termAn * termBn
           term2   = exp(I_dp*dble(m)*ax*xpq) * termC * termAm * termBm
           if (abs(term1) < tol)  exit
         ...
         do n      = 1 , Nmax
           ...
           termBn  = (cos(xqC)*cos(xqD)*bessi_scaled(n,B)-cos(ax*(2.d0*xq-xC-xD))*(0.25d0*(bessi_scaled(n-2,B)+2.d0*bessi_scaled(n,B)+bessi_scaled(n+2,B)))-I_dp/B*n*sin(ax*(2.d0*xq-xC-xD))*(0.5d0*(bessi_scaled(n-1,B)+bessi_scaled(n+1,B))))/ax/ax
           termAm  = bessi_scaled(n, A)
           termBm  = (cos(xqC)*cos(xqD)*bessi_scaled(n,B)-cos(ax*(2.d0*xq-xC-xD))*(0.25d0*(bessi_scaled(n-2,B)+2.d0*bessi_scaled(n,B)+bessi_scaled(n+2,B)))-I_dp/B*m*sin(ax*(2.d0*xq-xC-xD))*(0.5d0*(bessi_scaled(n-1,B)+bessi_scaled(n+1,B))))/ax/ax
           termc   = bessi_scaled(n, C) 
           term1   = exp(I_dp*dble(n)*ax*xpq) * termC * termAn * termBn
           term2   = exp(I_dp*dble(m)*ax*xpq) * termC * termAm * termBm
                     if (abs(term1) < tol)  exit
 ...
 if (abs(term2) < tol)  exit 
         ...
         do n      = 1 , Nmax
           ...
           termAn  = (n*I_dp/A*cos(xpB)*bessi_scaled(n,A)+(0.5d0*(bessi_scaled(n-1,A)+bessi_scaled(n+1,A))) * sin(xpB))/ax
           termBn  = bessi_scaled(n, B)
           termAm  = (m*I_dp/A*cos(xpB)*bessi_scaled(n,A)+(0.5d0*(bessi_scaled(n-1,A)+bessi_scaled(n+1,A))) * sin(xpB))/ax
           termBm  = bessi_scaled(n, B)
           termc   = bessi_scaled(n, C) 
           term1   = exp(I_dp*dble(n)*ax*xpq) * termC * termAn * termBn
           term2   = exp(I_dp*dble(m)*ax*xpq) * termC * termAm * termBm
 ...
 if (abs(term2) < tol)  exit 
        ...
        case (1000) ! | px  s   s   s    ( 65) 
         ...
         do n      = 1 , Nmax
           ...
           termAn  = (n*I_dp/A*cos(xpA)*bessi_scaled(n,A)+(0.5d0*(bessi_scaled(n-1,A)+bessi_scaled(n+1,A))) * sin(xpA))/ax
           termBn  = bessi_scaled(n, B)
           termAm  = (m*I_dp/A*cos(xpA)*bessi_scaled(n,A)+(0.5d0*(bessi_scaled(n-1,A)+bessi_scaled(n+1,A))) * sin(xpA))/ax
           termBm  = bessi_scaled(n, B)
           termc   = bessi_scaled(n, C) 
           term1   = exp(I_dp*dble(n)*ax*xpq) * termC * termAn * termBn
           term2   = exp(I_dp*dble(m)*ax*xpq) * termC * termAm * termBm
 ...
 if (abs(term2) < tol)  exit 
         ...
         do n      = 1 , Nmax
           ...
           termAn  = (cos(xpA)*cos(xpB)*bessi_scaled(n,A)-cos(ax*(2.d0*xp-xA-xB))*(0.25d0*(bessi_scaled(n-2,A)+2.d0*bessi_scaled(n,A)+bessi_scaled(n+2,A)))+I_dp/A*n*sin(ax*(2.d0*xp-xA-xB))*(0.5d0*(bessi_scaled(n-1,A)+bessi_scaled(n+1,A))))/ax/ax
           termBn  = bessi_scaled(n, B)
           termAm  = (cos(xpA)*cos(xpB)*bessi_scaled(n,A)-cos(ax*(2.d0*xp-xA-xB))*(0.25d0*(bessi_scaled(n-2,A)+2.d0*bessi_scaled(n,A)+bessi_scaled(n+2,A)))+I_dp/A*m*sin(ax*(2.d0*xp-xA-xB))*(0.5d0*(bessi_scaled(n-1,A)+bessi_scaled(n+1,A))))/ax/ax
           termBm  = bessi_scaled(n, B)
           termc   = bessi_scaled(n, C) 
           term1   = exp(I_dp*dble(n)*ax*xpq) * termC * termAn * termBn
           term2   = exp(I_dp*dble(m)*ax*xpq) * termC * termAm * termBm
...
! --------------------- !
 ...
 !if (abs(term2) < tol)  exit 
        ...
        ! end do
         ...
         n         = 0
         const     =  (pi * D)  *  (pi * D)   * exp(A+B-2.d0*(p+q)/(ax*ax))
         termAn    = (dcos(xpA)*dcos(xpB)*bessi_scaled(n,A)-dcos(ax*(2.d0*xp-xA-xB))*(0.25d0*(bessi_scaled(n-2,A)+2.d0*bessi_scaled(n,A)+bessi_scaled(n+2,A)))+I_dp/A*n*dsin(ax*(2.d0*xp-xA-xB))*(0.5d0*(bessi_scaled(n-1,A)+bessi_scaled(n+1,A))))/ax/ax
         termBn    = (dcos(xqC)*dcos(xqD)*bessi_scaled(n,B)-dcos(ax*(2.d0*xq-xC-xD))*(0.25d0*(bessi_scaled(n-2,B)+2.d0*bessi_scaled(n,B)+bessi_scaled(n+2,B)))-I_dp/B*n*dsin(ax*(2.d0*xq-xC-xD))*(0.5d0*(bessi_scaled(n-1,B)+bessi_scaled(n+1,B))))/ax/ax
         sum       =  const * bessi_scaled(n, C) * termAn * termBn 
         do n      = 1 , Nmax
           m       = - n 
           termAn  = (dcos(xpA)*dcos(xpB)*bessi_scaled(n,A)-dcos(ax*(2.d0*xp-xA-xB))*(0.25d0*(bessi_scaled(n-2,A)+2.d0*bessi_scaled(n,A)+bessi_scaled(n+2,A)))+I_dp/A*n*dsin(ax*(2.d0*xp-xA-xB))*(0.5d0*(bessi_scaled(n-1,A)+bessi_scaled(n+1,A))))/ax/ax
           termBn  = (dcos(xqC)*dcos(xqD)*bessi_scaled(n,B)-dcos(ax*(2.d0*xq-xC-xD))*(0.25d0*(bessi_scaled(n-2,B)+2.d0*bessi_scaled(n,B)+bessi_scaled(n+2,B)))-I_dp/B*n*dsin(ax*(2.d0*xq-xC-xD))*(0.5d0*(bessi_scaled(n-1,B)+bessi_scaled(n+1,B))))/ax/ax
           termAm  = (dcos(xpA)*dcos(xpB)*bessi_scaled(n,A)-dcos(ax*(2.d0*xp-xA-xB))*(0.25d0*(bessi_scaled(n-2,A)+2.d0*bessi_scaled(n,A)+bessi_scaled(n+2,A)))+I_dp/A*m*dsin(ax*(2.d0*xp-xA-xB))*(0.5d0*(bessi_scaled(n-1,A)+bessi_scaled(n+1,A))))/ax/ax
           termBm  = (dcos(xqC)*dcos(xqD)*bessi_scaled(n,B)-dcos(ax*(2.d0*xq-xC-xD))*(0.25d0*(bessi_scaled(n-2,B)+2.d0*bessi_scaled(n,B)+bessi_scaled(n+2,B)))-I_dp/B*m*dsin(ax*(2.d0*xq-xC-xD))*(0.5d0*(bessi_scaled(n-1,B)+bessi_scaled(n+1,B))))/ax/ax
           termc   = bessi_scaled(n, C) 
           term1   = exp(I_dp*dble(n)*ax*xpq) * termC * termAn * termBn
           term2   = exp(I_dp*dble(m)*ax*xpq) * termC * termAm * termBm
                     if (abs(term1) < tol)  exit
