...
subroutine integrate_ERI_mod_mod(pattern_id,p,q,p_x,q_x,phi,xpA,xpB,xqC,xqD,xa,xb,xc,xd,xp,xq,result,der)
      ...
      !call dqagi(f_decay_der, bound, inf, epsabs, epsrel, der   , abserr, neval, ier,Limit,Lenw,Last,Iwork,Work)
      ...
      function f_decay_der(t) result(ft)
        double precision, intent(in) :: t
        double precision             :: ft

         ft =  S_der(t)

      end function f_decay_der
      ...
      double precision function S_der(t) result(sum)
      ...
      case (0000) ! | s   s   s   s    ( 1 ) 
        ...
        do n      = 1 , Nmax
          ...
          if (abs(term1) < tol) exit
          if (abs(term2) < tol) exit
          sum     = sum + 2.d0*dble(n)*dsin(dble(n)*phi) * K * const
        end do
