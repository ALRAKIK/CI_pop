subroutine integrate_ERI_sum(pattern_id,p,q,p_x,q_x,phi,xpA,xpB,xqC,xqD,xa,xb,xc,xd,xp,xq,result)
      ...
      COMPLEX(KIND=KIND(1.0D0))            :: rot_factor , current_phase
      ...
      Nmax        = Peak+10
      rot_factor    = exp(I_dp * phi)
      current_phase = rot_factor
      do n = 1 , Nmax
        termAn  = iv_scaled(n, A)
        termBn  = iv_scaled(n, B)
        termc   = iv_scaled(n, C)
        term    = termC * termAn * termBn
        if (abs(term * const) < eps * dabs(sum) ) exit
        sum = sum + 2.d0 * const * ( &
              real(term) * real(current_phase) - &
              aimag(term) * aimag(current_phase) )
        current_phase = current_phase * rot_factor
      end do
      ...
      current_phase = rot_factor
      do n = 1 , Nmax
        termAn  = inv_ax * 0.5d0 * (sxpa * (iv_scaled(n-1,A)+iv_scaled(n+1,A)))
        termBn  = iv_scaled(n, B)
        termc   = iv_scaled(n, C) 
        term    = termC * termAn * termBn
        if (abs(term * const) < eps * dabs(sum) ) exit
